...
def generate(params):
    ...
    config = OmegaConf.load(f"{params.config}")
    model = load_model_from_config(config, f"{params.ckpt}")
...
class guiMain(tk.Frame):
    ...
    def create_widgets(self):
        self.keyword_textbox = guiTextEntry(master=self, label=u'キーワード', inittext=self.keyword, boxwidth=80)
        self.outfolder_dirdiag = guiDirDiag(master=self, label=u'出力フォルダ', initdir=self.outfolder)
        self.keyword_textbox.pack(anchor=tk.W)
        self.outfolder_dirdiag.pack(anchor=tk.W)
        frame_bottom = tk.Frame(master=self)
        button1 = tk.Button(master=frame_bottom, text="実行", command=self.generate)
        button2 = tk.Button(master=frame_bottom, text=("閉じる"), command=quit)
        button1.grid(row=0, column=0)
        button2.grid(row=0, column=1)
        frame_bottom.pack()
    def generate(self):
        self.keyword = self.keyword_textbox.entry.get()
        self.outfolder = self.outfolder_dirdiag.entry.get()
...
class Parameters:
    def __init__(self, initargs=None):
        parser = argparse.ArgumentParser(description='Stable Diffusion FrontEnd')
        parser.add_argument(
            '--keyword', '-k',
            help='keyword',
            default=''
